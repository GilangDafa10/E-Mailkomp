...
const Jaringan = () => {
    // State untuk menampung data departemen jaringan
    const [jaringanData, setJaringanData] = useState(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const cachedData = sessionStorage.getItem("jaringanData");
                let departments;
                ...
                const targetDept = departments.find(
                    (d) => d.slug === "jaringan" || d.name === "Jaringan"
                );
                setJaringanData(targetDept);
            } catch (err) {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-primary-dark text-white">
                {error ? (
                    ...
                    <span>Memuat Bidang Jaringan......</span>
                )}
            </div>
        );
    }

    const { head_of_bidang, divisions = [] } = jaringanData;
