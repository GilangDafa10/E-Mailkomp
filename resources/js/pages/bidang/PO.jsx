...
const POK = () => {
    // State untuk menampung data departemen POK
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const cachedData = sessionStorage.getItem("POKData");
                let departments;
                ...
                const targetDept = departments.find(
                    (d) =>
                        d.slug === "pok" ||
                        d.name === "Pengembangan Organisasi & Kewirausahaan"
                );
                setPOKData(targetDept);
            } catch (err) {
    ...
    const { head_of_bidang, divisions = [] } = POKData;
