...
const LitBang = () => {
    // State untuk menampung data departemen LitBang
    const [LitBangData, setLitBangData] = useState(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const cachedData = sessionStorage.getItem("LitBangData");
                let departments;
                ...
                const targetDept = departments.find(
                    (d) =>
                        d.slug === "penelitian-pengembangan" ||
                        d.name === "Penelitian & Pengembangan"
                );
                setLitBangData(targetDept);
            } catch (err) {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-primary-dark text-white">
                {error ? (
                    ...
                    <span>Memuat Bidang Literasi & Pengembangan......</span>
                )}
            </div>
        );
    }

    const { head_of_bidang, divisions = [] } = LitBangData;
